...
void WriteContigSetLongRemoveTail(ContigSet * contigSet, char * str, long int kmerLength){
    long int p = 0;
    ofstream ocout;
    ocout.open(str);
    ...
    while(contigSet!=NULL){
        if(contigSet->contig!=NULL){
            long int len = strlen(contigSet->contig);
            ocout<<">"<<p<<":"<<len<<endl;
            char * temp = new char[len-2*kmerLength+1];
            strncpy(temp, contigSet->contig+kmerLength, len-2*kmerLength);
            temp[len-2*kmerLength] = '\0';
            ocout<<temp<<endl;
            delete [] temp;
            temp = NULL;
            p++;
...
void ContigSetRemoveTail(ContigSet * contigSet, long int kmerLength){
    long int p = 0;
    while(contigSet!=NULL){
        if(contigSet->contig!=NULL){
            long int len = strlen(contigSet->contig);
