...
int ToUpperFa(char * source, char * result){
    long int i = 0;
    long int j = 0;
    long int t = 0;
    ...
    icin.open(source);
    ocout.open(result);
    char * contig = new char[5000000];
    while(icin.getline(contig, 5000000)){
        if(i%2!=0){
            long int contigLength = strlen(contig);
            for(j=0;j<contigLength;j++){
                contig[j] = toupper(contig[j]);
            }
            ocout<<">"<<t<<endl;
            ocout<<contig<<endl;
            t++;
...
typedef struct BcalmKmer{
    char * kmer;
    
}BcalmKmer;
/*
DBGraphHead * GetSimplePathFromBcalm(char * simplePathFile, char * deBruijnGraph, long int graphNode, long int kmerLength, int threadNumber){
    long int i = 0;
    long int j = 0;
    
    char * fileName1 = new char[20];
    strcpy(fileName1,"upFile.fa");      
    
    ToUpper(simplePathFile, fileName1);
    
    char * outputRight = new char[30];
    strcpy(outputRight, "rightKmer.fa");
    
    char * contig = new char[150000];
    
    DBGraphHead * deBruijnGraphHead = new DBGraphHead;
    deBruijnGraphHead->deBruijnGraph = new DBGraph[graphNode];
    deBruijnGraphHead->nodeNumber = graphNode;
      
    FILE * fp = fopen(fileName1,"r+");
    FILE * fp1 = fopen(outputRight, "w+");
   
    char * kmer = new char[kmerLength+1];  
    long int contigLen = 0;
    while(fgets(contig, 150000, fp)){
        contigLen = strlen(contig)-2;
        deBruijnGraphHead->deBruijnGraph[i].contig = new char[contigLen+1];  
        contig[contigLen] = '\0';      
        strcpy(deBruijnGraphHead->deBruijnGraph[i].contig, contig);
        deBruijnGraphHead->deBruijnGraph[i].contig[contigLen] = '\0';
        strncpy(kmer, contig + contigLen - kmerLength, kmerLength);
        kmer[kmerLength]='\0';
        fprintf(fp1,">%ld\n",j);
        fprintf(fp1,"%s\n",kmer);
        fflush(fp1);
    ...
    fclose(fp1);
    long int setNumber = 1;
    ReadSet * readSet = new ReadSet[setNumber];
    for(i = 0; i<setNumber;i++){
        readSet[i].address = outputRight;       
    }
    
    InitReadSet(readSet, setNumber, threadNumber);
    

    for(i=0;i<graphNode;i++){
        strncpy(kmer, deBruijnGraphHead->deBruijnGraph[i].contig, kmerLength);
        kmer[kmerLength] = '\0';
        
        SameReadIndex * temp = SearchAllSameRead(kmer,i,readSet);
        ...
        while(temp!=NULL){
            InsertLeftMateNode(deBruijnGraphHead->deBruijnGraph,i,temp->index);
            InsertRightMateNode(deBruijnGraphHead->deBruijnGraph,temp->index,i);          
            temp = temp->next;
        }
    ...
    for(i=0;i<5;i++){
        RemoveTip(deBruijnGraphHead->deBruijnGraph,kmerLength, graphNode);
        SimplePathMerge(deBruijnGraphHead->deBruijnGraph,kmerLength,graphNode);
        AdjustDBG(deBruijnGraphHead->deBruijnGraph,kmerLength,graphNode);
    }
    
    RemoveCycle1(deBruijnGraphHead->deBruijnGraph,kmerLength,graphNode);
    SimplePathMerge(deBruijnGraphHead->deBruijnGraph,kmerLength,graphNode);
    AdjustDBG(deBruijnGraphHead->deBruijnGraph,kmerLength,graphNode);
    
    WriteDBGraph(deBruijnGraphHead->deBruijnGraph, graphNode, deBruijnGraph);
    remove(fileName1);
    remove(outputRight);
...
*/
void InitDBGraph1(ReadSet * readSet, int kmerLength, KmerSetHashTableHead* kmerSetHashTableHead,DBGraphHead * deBruijnGraphHead, int totalThreadNumber){
    unsigned long int i = 0;
    unsigned long int j = 0;
    int num = 0;
    KmerSetHashTable* kmerSetHashTable = kmerSetHashTableHead->kmerSetHashTable;
...
void OptimizeDBGraphFromEdgeKmer(KmerSetHashTableHead * kmerSetHashTableHead,DBGraphHead * deBruijnGraphHead,long int kmerLength, char * graph){
    ...
    for(i=0;i<graphNodeCount;i++){
        GraphNode * out = deBruijnGraph[i].outNode;
        long int outNodeCount = NodeCount(out);
        long int * deleteNodeIndex = new long int[outNodeCount];
        for(j=0;j<outNodeCount;j++){
            deleteNodeIndex[j] = -1;
        }
        j = 0;
        while(out != NULL){
            tempKmer[0] = deBruijnGraph[i].contig[strlen(deBruijnGraph[i].contig)-kmerLength+1];
            strncpy(tempKmer+1,deBruijnGraph[out->index].contig,kmerLength-1);
            ...
            if(hashP ==0){
                ReverseComplement(tempKmer);
                long int hashP1 = KmerSetHashTableSearch(tempKmer,kmerSetHashTableHead->kmerSetHashTable,kmerLength,kmerSetHashTableHead->kmerSetHashTableCount);
                if(hashP1 == 0){
                    deleteNodeIndex[j] = out->index;
                    j++;
                }
            }
            out = out->next;
        }
        for(long int p = 0; p<j;p++){
            DBGRemoveOutNode(deBruijnGraph + i, deleteNodeIndex[p]);
            DBGRemoveInNode(deBruijnGraph + deleteNodeIndex[p], i);
        }
        delete [] deleteNodeIndex;
        deleteNodeIndex = NULL;
    ...
    SimplePathMerge(deBruijnGraph,kmerLength,graphNodeCount); 
    cout<<"aa"<<endl;
    AdjustDBG(deBruijnGraph,kmerLength,graphNodeCount);
    for(i=0;i<5;i++){
        RemoveTip(deBruijnGraph,kmerLength, graphNodeCount);
        SimplePathMerge(deBruijnGraph,kmerLength,graphNodeCount);
        AdjustDBG(deBruijnGraph,kmerLength,graphNodeCount);
    }
    ...
    WriteDBGraph(deBruijnGraphHead->deBruijnGraph, deBruijnGraphHead->nodeNumber, graph);
