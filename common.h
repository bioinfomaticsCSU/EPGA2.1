...
void TOLower(char * str, long int length){
    for(long int i = 0; i<length;i++){
        str[i] = tolower(str[i]);
    }
}

void TOUpper(char * str, long int length){
    for(long int i = 0; i<length;i++){
        str[i] = toupper(str[i]);
    }
}
...
bool ReverseComplement(char * temp1, char * temp2){
    ...
    for(i=0;i<len;i++){
        if(temp1[i]=='A'){
            ...
            cout<<"Contain other charaters not (ATGCN) !"<<endl;
            return false;
        }
    }
    temp2[len]='\0';
