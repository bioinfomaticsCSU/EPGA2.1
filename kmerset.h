...
unsigned long long int KmerHashTableSearch(char * value, KmerHashTable * kmerHashTable, unsigned int kmerLength, unsigned long int kmerHashTableCount){
    ...
    int p = 0;
    i = Hash(value, kmerLength, kmerHashTableCount);
...
int SingleKmerCopyToDisc(KmerSetHashTable * kmerSetHashTable, unsigned int kmerLength, unsigned long long int kmerSetHashTableCount, char * address){
    ...
    char * tempRC = new char[kmerLength + 1];
    ...
    for(i = 0; i < kmerSetHashTableCount; i++){
        if(kmerSetHashTable[i].kmer!=0){                 
            ...
            ReverseComplement(temp11, tempRC);
            long int hashP = KmerSetHashTableSearch(tempRC,kmerSetHashTable,kmerLength,kmerSetHashTableCount);
            TOLower(temp11,kmerLength);
            fprintf(fp,"%s;\n",temp11);
            if(hashP == 0){
                TOLower(tempRC,kmerLength);
                fprintf(fp,"%s;\n",tempRC);
            }        
        }
    }
...
void * GetKmerSetAddressThread(void * arg){
	...
	long int tempFrequency = 0;
	char * tempKmer1 = new char[kmerLength + 1];
	char * kmerString = new char[10];
	
    ifstream icin;
    icin.open(getKmerSetAddressP->address);
	//cout<<getKmerSetAddressP->address<<"--"<<getKmerSetAddressP->totalThreadNumber<<"--"<<getKmerSetAddressP->threadIndex<<endl;
	long int contigLength = 0;
	long int length = kmerLength + 100;
	char * contig = new char[length + 1];
	
	while(icin.getline(contig, length)){
		if(i%getKmerSetAddressP->totalThreadNumber == getKmerSetAddressP->threadIndex){     
			contigLength = strlen(contig);
			//cout<<"bb--"<<contigLength<<endl;
			strncpy(tempKmer1, contig, kmerLength);
			tempKmer1[kmerLength]='\0';
			//cout<<"cc--"<<tempKmer1<<endl;
			strncpy(kmerString, contig+kmerLength+1, contigLength-kmerLength-1);
			kmerString[contigLength-kmerLength-1] = '\0';
			//cout<<"dd--"<<kmerString<<endl;
			tempFrequency = atoi(kmerString);	
			//cout<<"ee--"<<tempFrequency<<endl;
            //cout<<contig<<"--"<<contigLength<<"--"<<kmerString<<"--"<<tempFrequency<<endl;	
...
unsigned long int GetKmerSetAddress(char * address, KmerSetHashTable * kmerSetHashTable, unsigned long int kmerSetHashTableCount, int kmerLength, int setNumber, int setIndex, int totalThreadNumber){
    ...
    for(i = 0; i< totalThreadNumber; i++){
        ...
        //cout<<kmerSetHashTableCount<<"--"<<kmerLength<<"--"<<setNumber<<"--"<<setIndex<<"--"<<totalThreadNumber<<endl;     
    ...
    for(i = 0; i<kmerSetHashTableHead->setNumber; i++){
        ...
        for(j = 0; j<kmerSetHashTableHead->kmerSetHashTableCount; j++){
            if(kmerSetHashTableHead->kmerSetHashTable[j].frequency!=NULL&&kmerSetHashTableHead->kmerSetHashTable[j].frequency[i]!=0){
                ...
                //cout<<kmerSetHashTableHead->kmerSetHashTable[j].frequency[i]<<endl;
				num++;
            }
        }
...
void InitKmerSet(KmerSet * kmerSet, KmerSetHashTableHead * kmerSetHashTableHead, int kmerLength, unsigned long long int kmerSetHashTableCount, int setNumber, unsigned int totalThreadNumber, char * singleKmerAddress, char * kmerHashTableAddress){
    int i = 0;
    unsigned long long int j = 0;
    //cout<<"ffff---"<<kmerSetHashTableCount<<endl;
    ...
    for(i = 0; i<setNumber; i++){
        //cout<<"--"<<kmerSet[i].address<<endl;
        GetKmerSetAddress(kmerSet[i].address, kmerSetHashTable, kmerSetHashTableCount, kmerLength, setNumber, i, totalThreadNumber); 
        //cout<<"endl"<<endl;
    }  
    
    /*
    char * tempKmer = new char[100];
    strcpy(tempKmer, "AATATCAAATACTACTAAGAA");
    
    long int hashP = KmerSetHashTableSearch(tempKmer,kmerSetHashTable,globalKmerLength,kmerSetHashTableCount);
    long int avgKmerNum = 0;
    if(hashP!=0){
        avgKmerNum = kmerSetHashTable[hashP-1].frequency[0];
    }
    cout<<avgKmerNum<<endl;
    */
	SingleKmerCopyToDisc(kmerSetHashTable, kmerLength, kmerSetHashTableCount,singleKmerAddress);
	CopyToDisc2(kmerSetHashTable,setNumber, kmerLength, kmerSetHashTableCount,kmerHashTableAddress);
}


void InitKmerSet(ReadSet * readSet, KmerSet * kmerSet, KmerSetHashTableHead * kmerSetHashTableHead, int kmerLength, int setNumber, unsigned int totalThreadNumber,  char * singleKmerAddress, char * kmerHashTableAddress){
    int i = 0;
    unsigned long long int j = 0;
...
void InitKmerSetOptimizeDBGraph(KmerSet * kmerSet, KmerSetHashTableHead * kmerSetHashTableHead, int kmerLength, unsigned long long int kmerSetHashTableCount, int setNumber, unsigned int totalThreadNumber){
    int i = 0;
    unsigned long long int j = 0;
    KmerSetHashTable * kmerSetHashTable = new KmerSetHashTable[kmerSetHashTableCount];
    kmerSetHashTableHead->setNumber = setNumber;
