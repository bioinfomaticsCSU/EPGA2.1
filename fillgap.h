...
void DeleteGapRegionContigSet(GapRegionContigSet * gapRegionContigSet, long int number){
    
    long int i = 0;
    for(i=0;i<number;i++){
        GapRegionContig * gapRegionContig = gapRegionContigSet[i].gapRegionContig;
        ContigSet * tempContigSet = gapRegionContig->contigSet;
        ContigSet * temp = tempContigSet;
        ...
        delete [] gapRegionContig->right;
        gapRegionContig->right = NULL;
    }
    
    delete [] gapRegionContigSet;
    gapRegionContigSet = NULL;
...
char * MergeGapContig(char * left, char * mid, char * rightCut, char * right, long int overlap, long int overlap1){
     ...
     char * result = NULL;
     //cout<<"length:"<<leftLength<<"--"<<midLength<<"--"<<rightLength<<"--"<<endl;
     if(rightCut == NULL){
         //cout<<"ttt0--"<<leftLength + midLength + rightLength - overlap - overlap1 - 2*cut + 1<<endl;
         result = new char[leftLength + midLength + rightLength - overlap - overlap1 - 2*cut + 1];
         strncpy(result, left, leftLength);
         strncpy(result + leftLength - overlap - cut, mid, midLength);
         //result = (char *)realloc(left, leftLength + midLength + rightLength - overlap - overlap1 - 2*cut + 1);
         //cout<<"ttt1--"<<leftLength - overlap - cut<<endl;
         //result[leftLength - overlap - cut] = '\0';
         //strcat(result, mid);
         long int start = leftLength + midLength - overlap - overlap1 - cut;
         //cout<<"ttt2--"<<start<<endl;
         ...
         result[start] = '\0';
         //cout<<result<<endl;
         //cout<<"tttendddddddd"<<endl;
     }else{
         //cout<<"sss0"<<endl;
         long int rightCutLength = strlen(rightCut);
         //cout<<"sss1---"<<rightCutLength<<"--"<<leftLength + midLength + rightCutLength + rightLength - overlap - overlap1 - 2*cut + 1<<endl;
         //cout<<"sss1111---"<<leftLength - overlap - cut<<endl;
         result = new char[leftLength + midLength + rightCutLength + rightLength - overlap - overlap1 - 2*cut + 1];
         //cout<<"sss2ttt--"<<leftLength + midLength - overlap - overlap1 - cut<<endl;
         strncpy(result, left, leftLength);
         //cout<<"sss33ttt--"<<endl;
         strncpy(result+leftLength - overlap - cut, mid, midLength);
         //cout<<"sss44ttt--"<<endl;
         ...
         char * tempRightCut = new char[rightCutLength + rightLength - cut + 1];
         strncpy(tempRightCut, rightCut, rightCutLength);
         delete [] rightCut;
         rightCut = tempRightCut;
         tempRightCut = NULL;
         //rightCut = (char *)realloc(rightCut, rightCutLength + rightLength - cut + 1);
         long int start = rightCutLength;
         //cout<<"sss3--"<<start<<endl;
         for(i=cut;i<rightLength;i++){
         ...
         rightCut[start] = '\0';
         //cout<<rightCut<<endl;
         strncpy(result + leftLength + midLength - overlap - overlap1 - cut, rightCut, rightCutLength + rightLength -cut);
         result[leftLength + midLength + rightCutLength + rightLength - overlap - overlap1 - 2*cut] = '\0';
        ...
        while(temp->next!=NULL){
            if(gapRegionContigSet[j].gapRegionContig!=NULL){
                ...
                if(lastContigSet->next!=NULL){
                    ...
                    tempN[ss] = '\0';
                    char * tempContig = new char[strlen(temp->contig) + strlen(temp->next->contig) + gapDistance +1];
                    strncpy(tempContig, temp->contig, strlen(temp->contig));
                    strncpy(tempContig+strlen(temp->contig),tempN,gapDistance);
                    strncpy(tempContig+strlen(temp->contig)+gapDistance,temp->next->contig,strlen(temp->next->contig));
                    tempContig[strlen(temp->contig) + strlen(temp->next->contig) + gapDistance] = '\0';
                    //cout<<tempContig<<endl;
                    delete [] temp->contig;
                    temp->contig = tempContig;
                    tempContig = NULL;
                    ...
                    char * mid = lastContigSet->contig;
                    //cout<<"dd2--"<<gapRegionContigSet[j].gapRegionContig->overlap<<endl;
                    if(gapRegionContigSet[j].gapRegionContig->overlap > 0){
                        long int overlap = gapRegionContigSet[j].gapRegionContig->overlap;
                        char * tempContig0 = new char[strlen(temp->contig) + strlen(temp->next->contig) - overlap - 2*cut + 1];
                        strncpy(tempContig0, temp->contig, strlen(temp->contig));
                        //temp->contig = (char *)realloc(temp->contig, strlen(temp->contig) + strlen(temp->next->contig) - overlap - 2*cut + 1);
                        //temp->contig[strlen(temp->contig) - cut - overlap] = '\0';
                        char * tempContig = new char[strlen(temp->next->contig) - cut + 1];
                        SubContig(tempContig, temp->next->contig, cut, strlen(temp->next->contig));
                        strncpy(tempContig0+strlen(temp->contig)- cut - overlap, tempContig, strlen(temp->next->contig) - cut);
                        tempContig0[strlen(temp->contig) + strlen(temp->next->contig) - overlap - 2*cut] = '\0';
                        //cout<<tempContig0<<endl;
                        delete [] temp->contig;
                        temp->contig = tempContig0;
                        tempContig0 = NULL;
                        //strcat(temp->contig, tempContig);
                        delete [] tempContig;
                        tempContig = NULL;                   
                ...
                tempN[ss] = '\0';
                //cout<<"dfdfdfdfd---"<<gapDistance<<endl;
                //cout<<temp->contig<<endl;
                //cout<<strlen(temp->contig)<<endl;
                //cout<<temp->next->contig<<"--"<<strlen(temp->next->contig)<<endl;
                //cout<<strlen(temp->contig) + strlen(temp->next->contig) + gapDistance +1<<endl;
                char * tempContig = new char[strlen(temp->contig) + strlen(temp->next->contig) + gapDistance +1];
                //cout<<"fffff0"<<endl;
                strncpy(tempContig, temp->contig, strlen(temp->contig));
                //cout<<"fffff1"<<endl;
...
void FillGap(ScaffoldSetHead * scaffoldSetHead, ReadSet * readSet, int setNumber, KmerSetHashTableHead * kmerSetHashTableHead, int kmerLength, int totalThreadNumber){
    ...
    //DeleteGapRegionContigSet(gapRegionContigSet, scaffoldSetHead->gapNumber);
