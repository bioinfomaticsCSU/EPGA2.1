...
void InitReadSet(ReadSet * readSet, int readSetNumber, unsigned int totalThreadNumber){
    ...
    for(i = 0; i<readSetNumber; i++){
        ...
        char * tempReverseComplement = new char[100];
        strcpy(temp, "TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT");
        ReverseComplement(temp, tempReverseComplement);
       
        ReadMate * tempReadMate = SearchRightReadMate(temp, readSet+i, 0);  
        ReadMate * tempReadMate1 = SearchLeftReadMate(tempReverseComplement, readSet+i, 1);
        
        
        if(tempReadMate!=NULL){
            ReadMate * temp1 = tempReadMate;
            tempReadMate = tempReadMate->next;
            delete temp1;
        }
        if(tempReadMate1!=NULL){
            ReadMate * temp1 = tempReadMate1;
            tempReadMate1 = tempReadMate1->next;
            delete temp1;
        }
        ReadMate * tempReadMate2 = tempReadMate;
        if(tempReadMate == NULL){
            tempReadMate = tempReadMate1;
        }else{
            while(tempReadMate2->next != NULL){
                tempReadMate2 = tempReadMate2->next;
            }
            tempReadMate2->next = tempReadMate1;
        }
        if(tempReadMate==NULL){
            cout<<"error"<<endl;
            exit(0);
            ...
            while(tempReadMate!=NULL){
                cout<<tempReadMate->readMate<<endl;
                cout<<"ffff"<<endl;
                tempReadMate = tempReadMate->next;
            }
        }
