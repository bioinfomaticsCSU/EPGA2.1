...
SameReadIndex * SearchAllSameRead(char * read, long int readIndex, ReadSet * readSet){
    ...
    //cout<<"tt"<<endl;
    SameReadIndex * head = new SameReadIndex;
    for(j = mid; j < readSet->readNumber && i == readSet->readHashTable[j].keyValue; j++){
        
        start = readSet->readLength * (readSet->readHashTable[j].index);
        //cout<<"tt2:"<<readSet->readHashTable[j].index<<"--"<<readSet->readLength<<"--"<<read<<endl;
        index = 0;
        
        //for(index = 0; index<readSet->readLength; index++){
            //cout<<GetBit(readSet->readSet, start+index);
        //}
        //cout<<endl;
        ...
        if(index != -1 && readSet->readHashTable[j].index != readIndex){
            //cout<<"ff"<<endl;
            SameReadIndex * temp = new SameReadIndex;
            temp->index = readSet->readHashTable[j].index;
...
void * GetReadNumberThread(void * arg){
    ...
    unsigned long int i = 0;
	char temp[1000];   	
	FILE * fp = fopen(getReadNumberP->address,"r");
	//cout<<"ttttttt--"<<getReadNumberP->address<<endl;
	if(fp==NULL){
        cout<<"error"<<endl;
	}
	while(NULL!=fgets(temp,1000,fp)){
		//cout<<temp<<endl;
		if(i%4==0&&((unsigned long int)(i/4)%getReadNumberP->totalThreadNumber == getReadNumberP->threadIndex)){		    
...
void * GetReadSetThread(void * arg){
	...
	char temp1[readLength + 2];
	FILE * fp = fopen(getReadSetP->readSet->leftAddress,"r"); 
	FILE * fp1 = fopen(getReadSetP->readSet->rightAddress,"r");
    unsigned long int p = 2*readLength * (getReadSetP->threadIndex);
	while(NULL!=fgets(temp,1000,fp)){
        fgets(temp1,1000,fp1);
		if(i%4 == 1 && (i/4)%getReadSetP->totalThreadNumber == getReadSetP->threadIndex){
            temp[readLength] = '\0';
            temp1[readLength] = '\0';
            if(getReadSetP->readSet->orientation == 1){             
                ReverseComplement(temp);
                ReverseComplement(temp1);
			}	
			pthread_mutex_lock(&mutex3);
            SetBit(getReadSetP->readSet->readSet, p, readLength, temp);
            p = p + readLength;
            SetBit(getReadSetP->readSet->readSet, p, readLength, temp1);
            pthread_mutex_unlock(&mutex3);
            
            ReadHashTableInsert(getReadSetP->readSet, i/2, temp);
            ReadHashTableInsert(getReadSetP->readSet, (i/2)+1, temp1);
            p = p + 2*readLength * getReadSetP->totalThreadNumber - readLength;		    
		}	
		i++;	
	}
...
void InitReadSet(ReadSet * readSet, int readSetNumber, unsigned int totalThreadNumber){
    ...
    for(i = 0; i<readSetNumber; i++){
        readSet[i].readNumber = 2*GetReadNumber(readSet[i].rightAddress, totalThreadNumber);
        readSet[i].readLength = GetReadOrKmerLength(readSet[i].rightAddress);
        //cout<<readSet[i].readLength<<"--"<<readSet[i].readNumber<<endl;
    }
    ...
    strcpy(tempRead, "GCTTCTTTCACTTTATTCACCAACTG");
    cout<<SearchReadNumber(tempRead, readSet+1)<<endl;
    ReadMate * tempMate = NULL;
    tempMate = SearchRightReadMate(tempRead, readSet+1);
    tempMate = tempMate->next;
    while(tempMate!=NULL){
        cout<<"--"<<tempMate->readMate<<endl;
        tempMate=tempMate->next;
    }
    exit(0);
    */
